...
def expand(path, map):
    ...
    path_list = []
    nearbyStations = map.connections[path.route[-1]]
...
def remove_cycles(path_list):
    ...
    for path in path_list:
        if len(path.route) == len(set(path.route)): #set contains only unique elemets.
            noDuplicates.append(path)
...
def insert_depth_first_search(expand_paths, list_of_path):
    ...
    for p in expand_paths:
        expands = p.route
        expands.reverse()
        newList.append(Path(expands))
            
    finalList = newList + list_of_path
    return finalList
...
def depth_first_search(origin_id, destination_id, map):
    ...
    while stations[0].route[0] != destination_id and len(stations) != 0:
        head = stations[0].route[0]
        stations[0].route.reverse() #explained why at the return statment
    ...
    if not stations:
        ...
        """ At class we were told to sort it from destination to origin, that's why I have to reverse it now """
        stations[0].route.reverse()
        return stations[0]
...
def coord2station(coord, map):
    ...
    for x, y in map.stations.items():
       dist = math.hypot(y['x'] - coord[0], y['y'] - coord[1])
       if dist <= min:
           if dist < min:
               nearby = []
           nearby.append(x)
